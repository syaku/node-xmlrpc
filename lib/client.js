...
Client.prototype.methodCall = function methodCall(method, params, callback) {
  ...
  var request = transport.request(options, function(response) {
    if (response.statusCode == 404) {
      ...
      if(auth && auth.algorithm){
        ha1 = sechash.basicHash(auth.algorithm, options.digest_auth.user+":" + auth.realm + ":"+options.digest_auth.pass);
        ha2 = sechash.basicHash(auth.algorithm, request.method+":" + options.path);
        res = sechash.basicHash(auth.algorithm, "" + ha1 + ":" + auth.nonce + ":1::auth:" + ha2);
        authRequestParams = {
